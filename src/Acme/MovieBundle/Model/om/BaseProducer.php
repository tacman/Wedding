...
/**
 * Base class that represents a row from the 'producers' table.
 *
 * 
 *
...
abstract class BaseProducer extends BaseObject  implements Persistent
{
	 ...
	 * Peer class name
	 */
	const PEER = 'Acme\\MovieBundle\\Model\\ProducerPeer';
	...
	protected function doInsert(PropelPDO $con)
	{
		...
		if ($this->isColumnModified(ProducerPeer::ID)) {
			$modifiedColumns[':p' . $index++]  = '`ID`';
		}
		if ($this->isColumnModified(ProducerPeer::NAME)) {
			$modifiedColumns[':p' . $index++]  = '`NAME`';
		}
		if ($this->isColumnModified(ProducerPeer::IS_PUBLISHED)) {
			$modifiedColumns[':p' . $index++]  = '`IS_PUBLISHED`';
		}

		$sql = sprintf(
			'INSERT INTO `producers` (%s) VALUES (%s)',
	...
	/**
	 * Sets a collection of Movie objects related by a one-to-many relationship
	...
	{
		$this->moviesScheduledForDeletion = $this->getMovies(new Criteria(), $con)->diff($movies);
