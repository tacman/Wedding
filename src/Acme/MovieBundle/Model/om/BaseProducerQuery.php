...
/**
 * Base class that represents a query for the 'producers' table.
 *
 * 
 *
...
abstract class BaseProducerQuery extends ModelCriteria
{
	 ...
	 */
	public function __construct($dbName = 'default', $modelName = 'Acme\\MovieBundle\\Model\\Producer', $modelAlias = null)
	{
	...
	public function findPk($key, $con = null)
	{
		...
		if ((null !== ($obj = ProducerPeer::getInstanceFromPool((string) $key))) && !$this->formatter) {
	 ...
	 *
	 * @return    Producer A model object, or null if the key is not found
	 */
	protected function findPkSimple($key, $con)
	{
		$sql = 'SELECT `ID`, `NAME`, `IS_PUBLISHED` FROM `producers` WHERE `ID` = :p0';
		try {
		...
		if ($row = $stmt->fetch(PDO::FETCH_NUM)) {
			$obj = new Producer();
			$obj->hydrate($row);
			ProducerPeer::addInstanceToPool($obj, (string) $row[0]);
		}
		$stmt->closeCursor();
