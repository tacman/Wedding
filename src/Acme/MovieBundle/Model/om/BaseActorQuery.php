...
abstract class BaseActorQuery extends ModelCriteria
{
	 ...
	 */
	public function __construct($dbName = 'default', $modelName = 'Acme\\MovieBundle\\Model\\Actor', $modelAlias = null)
	{
	...
	{
		$sql = 'SELECT `ID`, `NAME` FROM `actors` WHERE `ID` = :p0';
		try {
		...
		if ($row = $stmt->fetch(PDO::FETCH_NUM)) {
			$obj = new Actor();
			$obj->hydrate($row);
			ActorPeer::addInstanceToPool($obj, (string) $row[0]);
		}
		$stmt->closeCursor();
