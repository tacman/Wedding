...
abstract class BaseMovieQuery extends ModelCriteria
{
	 ...
	 */
	public function __construct($dbName = 'default', $modelName = 'Acme\\MovieBundle\\Model\\Movie', $modelAlias = null)
	{
	...
	public function findPk($key, $con = null)
	{
		...
		if ((null !== ($obj = MoviePeer::getInstanceFromPool((string) $key))) && !$this->formatter) {
	 ...
	 *
	 * @return    Movie A model object, or null if the key is not found
	 */
	protected function findPkSimple($key, $con)
	{
		$sql = 'SELECT `ID`, `TITLE`, `IS_PUBLISHED`, `RELEASE_DATE`, `PRODUCER_ID` FROM `movies` WHERE `ID` = :p0';
		try {
		...
		if ($row = $stmt->fetch(PDO::FETCH_NUM)) {
			$obj = new Movie();
			$obj->hydrate($row);
			MoviePeer::addInstanceToPool($obj, (string) $row[0]);
		}
		$stmt->closeCursor();
	 ...
	 * @param     string $joinType Accepted values are null, 'left join', 'right join', 'inner join'
	 *
	 * @return    \Acme\MovieBundle\Model\ProducerQuery A secondary query class using the current class as primary query
	 */
	public function useProducerQuery($relationAlias = null, $joinType = Criteria::INNER_JOIN)
	{
		return $this
			->joinProducer($relationAlias, $joinType)
			->useQuery($relationAlias ? $relationAlias : 'Producer', '\Acme\MovieBundle\Model\ProducerQuery');
	}
	 ...
	 * @param     string $joinType Accepted values are null, 'left join', 'right join', 'inner join'
	 *
	 * @return    \Acme\MovieBundle\Model\ActorHasMovieQuery A secondary query class using the current class as primary query
	 */
	public function useActorHasMovieQuery($relationAlias = null, $joinType = Criteria::INNER_JOIN)
	{
		return $this
			...
			->useQuery($relationAlias ? $relationAlias : 'ActorHasMovie', '\Acme\MovieBundle\Model\ActorHasMovieQuery');
	}

	/**
	 * Filter the query by a related Actor object
	 * using the actors_movies table as cross reference
	 *
